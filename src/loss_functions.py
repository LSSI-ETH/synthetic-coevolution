...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_sigmoid:
            self.cls_criterion = binary_cross_entropy_modified
        #else:
    ...
    def forward(self,
                cls_score,
                label,
                rbd_plm_ignored_label_idxs=None, # masked label training
                args=None,
                labels=None,
                weight=None,
                avg_factor=None,
            ...
            if args is not None:
                ...
                if 'rbd_plm' in args.basemodel:
                    if args.use_lmt == True:
                        ...
                        loss = loss.masked_fill_(rbd_plm_ignored_label_idxs, 0)
                        num_ignored_label_idxs = ~rbd_plm_ignored_label_idxs
                        num_ignored_label_idxs = num_ignored_label_idxs.sum()
                        
                        if args.loss_reduce == 'mean':
                            loss = loss.sum() / num_ignored_label_idxs
                        elif args.loss_reduce == 'sum':
                            loss = loss.sum()
                        
                    elif args.use_lmt == False:
                        unkown_label_mask = self.replace_label_values(label, 0,1,1)
    ...
    def logit_reg_functions(self, labels, logits, weight=None): 
        ...
        if 'neg_scale' in self.logit_reg:
            ...
            #logits = logits * neg_labels * self.neg_scale  + logits * pos_labels
            
            if weight is not None:
                weight = weight / self.neg_scale * (1 - labels) + weight * labels
                #weight = weight / self.neg_scale * neg_labels + weight * pos_labels
...
def binary_cross_entropy_modified(pred,
                         label,
                         weight=None,
                         reduction='mean',
                         avg_factor=None):
